...
from MahakRobot import telethn as Mahak
from MahakRobot.events import Mahakinline
from MahakRobot.events import register as Mahakbot
from MahakRobot.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await Mahak(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await Mahak(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@Mahakbot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
